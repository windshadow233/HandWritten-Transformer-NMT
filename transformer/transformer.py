    ...
    def predict_next(self, current_pred_sens, src_seq, encoder_output, beam_size=5):
        ...
        for sen, score in current_pred_sens:
            ...
            if sen.squeeze(0)[-1].item() == self.eos_idx:
                next_possible_sentences.append((sen, score))
                continue
            ...
            next_possible_sentences.extend([(torch.cat([sen[:, :-1], word[None, None]], 1), score + math.log(prob.item())) for word, prob in zip(max_prob_word, max_prob)])
        next_possible_sentences.sort(key=lambda x: x[1], reverse=True)
        return next_possible_sentences[:beam_size]

    def beam_search(self, input_seq, max_len=100, beam_size=3):
        """
        对一个句子进行解码
        :param input_seq: (L,)
        :param max_len: 最大句长
        :param beam_size: 单步搜索空间
        :return pred: beam_size个评分最优的翻译句子.
        """
